var punctuation = require('../src/punctuation');
...
describe('Punctuation', function() {
  ...
  it('text without punctuation symbols should be the same after call the function', function() {
    ...
    expect(punctuation(html)).to.equal( '<p>En un lugar de la mancha</p>');
  });
  ...
  it('should replace three dots by elipses symbol on texts with multiple matches', function() {
    ...
    expect(punctuation(html)).to.equal('<p>En un lugar de la mancha… De cuyo nombre no quiero acordarme… no ha mucho tiempo que vivía…</p>');
  });
  ...
  it('should replace two dashes (--) by a dash (–)', function() {
    ...
    expect(punctuation(html)).to.equal('<p>Pues aquí hay algo que no te va a gustar Laura, –dijo Carlos– yo también voy al equipo de Juan.</p>');
  });
  ...
  it('should replace a dash between spaces ( – ) by (&thinsp;&mdash;&thinsp;)', function() {
    ...
    expect(punctuation(html)).to.equal('<p>Use an "en" dash to con­nect num­bers in a range. It means "up to and includ­ing" when used like this: "During the years 1998&thinsp;&mdash;&thinsp;1999," and "peo­ple aged 55&thinsp;&mdash;&thinsp;63."</p>');
  });
